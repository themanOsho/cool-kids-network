...
class PropertyHook extends NodeAbstract implements FunctionLike {
	 ...
	 */
	public function __construct( $name, $body, array $subNodes = array(), array $attributes = array() ) {
	...
	public function getStmts(): ?array {
		if ( $this->body instanceof Expr ) {
			...
			if ( $name === 'get' ) {
				return array( new Return_( $this->body ) );
			}
			if ( $name === 'set' ) {
				...
				return array( new Expression( new Assign( $prop, $this->body ) ) );
			}
	...
	public function getSubNodeNames(): array {
		return array( 'attrGroups', 'flags', 'byRef', 'name', 'params', 'body' );
	}
}
