...
class NameContext {
	...
	protected array $origAliases = array();
	...
	public function startNamespace( ?Name $namespace = null ): void {
		...
		$this->origAliases = $this->aliases = array(
	 ...
	 */
	public function addAlias( Name $name, string $aliasName, int $type, array $errorAttrs = array() ): void {
	...
	public function getPossibleNames( string $name, int $type ): array {
		...
		if ( $type === Stmt\Use_::TYPE_NORMAL ) {
			...
			if ( $lcName === 'self' || $lcName === 'parent' || $lcName === 'static' ) {
				return array( new Name( $name ) );
			}
		}

		// Collect possible ways to write this name, starting with the fully-qualified name
		$possibleNames = array( new FullyQualified( $name ) );
