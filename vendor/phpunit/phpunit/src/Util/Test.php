...
final class Test {
	...
	public static function getDependencies( string $className, string $methodName ): array {
		...
		$dependsAnnotations = $annotations['class']['depends'] ?? array();
		...
		foreach ( array( 'method', 'class' ) as $element ) {
			if ( isset( $annotations[ $element ]['covers'] ) ) {
				foreach ( $annotations[ $element ]['covers'] as $coversTarget ) {
					$groups[] = array( '__phpunit_covers_' . self::canonicalizeName( $coversTarget ) );
				}
			}

			if ( isset( $annotations[ $element ]['uses'] ) ) {
				foreach ( $annotations[ $element ]['uses'] as $usesTarget ) {
					$groups[] = array( '__phpunit_uses_' . self::canonicalizeName( $usesTarget ) );
				}
			}
		}

		return array_unique( array_merge( array(), ...$groups ) );
	}
	...
	private static function getLinesToBeCoveredOrUsed( string $className, string $methodName, string $mode ): array {
		...
		$list = $annotations['class'][ $mode ] ?? array();

		if ( isset( $annotations['method'][ $mode ] ) ) {
			$list = array_merge( $list, $annotations['method'][ $mode ] );
		}

		$codeUnits = CodeUnitCollection::fromArray( array() );
		$mapper    = new Mapper();
