...
final class TestRunner extends BaseTestRunner {
		...
		if ( $codeCoverageReports > 0 || isset( $arguments['xdebugFilterFile'] ) ) {
			if ( isset( $arguments['coverageFilter'] ) ) {
				if ( ! is_array( $arguments['coverageFilter'] ) ) {
					$coverageFilterDirectories = array( $arguments['coverageFilter'] );
				} else {
	...
	private function handleConfiguration( array &$arguments ): void {
		...
		$arguments['testdoxExcludeGroups']  = $arguments['testdoxExcludeGroups'] ?? array();
		$arguments['testdoxGroups']         = $arguments['testdoxGroups'] ?? array();
